...
def draw_on_object(contours, img):
    if len(contours) != 0:
...
def check_angles():
    ...
    for i in range(20):     # capture x image
        _, img = video.read()

        image = cv2.cvtColor(img, cv2.COLOR_BGR2HSV)

        # pour avoir les masques
        mask_1 = red_mask(image)
        mask_2 = blue_mask(image)
        ...
        start = draw_on_object(contours_1, img)
        end = draw_on_object(contours_2, img)
...
if __name__ == '__main__':
    ...
    with concurrent.futures.ThreadPoolExecutor() as executor:
        future = executor.submit(check_angles)
    angle_1, angle_2, lenght = utils.unpack(future.result())
    print(angle_1)
    print(angle_2)
    print(lenght)